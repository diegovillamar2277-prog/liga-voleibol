...
// ============================================================
import { getLigaByCodigo } from '../lib/db.js';
import { sb } from '../lib/supabase.js';
import { esc, formatFecha, toast } from '../lib/ui.js';
...
export async function renderPublicView(container, codigoInicial = '') {
  container.innerHTML = `
    <div class="app-shell">
      ...
      <div id="pub-body">
        ${codigoInicial ? '<div class="loading-spinner" style="margin:3rem auto"></div>' : renderBuscador()}
      </div>
    </div>`;
...
function renderTablaPublica(el, equipos, partidos, cfg) {
  ...
  if (!tabla.length) {
    el.innerHTML = '<p class="empty">Aún no hay equipos registrados.</p>';
    return;
  }
    ...
    <div class="tabla-wrap">
      <table class="tabla-pos">
        <thead><tr>
          ...
          ${usarSets ? '<th title="Sets ganados">SG</th><th title="Sets perdidos">SP</th>' : ''}
          ${mostrarDS ? '<th title="Diferencia de sets">DS</th>' : ''}
          ${usarPts  ? '<th title="Puntos">PTS</th>' : ''}
        </tr></thead>
        <tbody>
          ${tabla.map((r, i) => {
            const ds = r.sg - r.sp;
...
function renderResultados(el, partidos, cfg) {
  ...
  if (!norm.length) {
    el.innerHTML = '<p class="empty">No hay partidos registrados aún.</p>';
    return;
  }
  ...
  el.innerHTML = `
    <div class="fixture-list">
      ${norm.map(p => {
        ...
        const sets = usarSets && p.sets?.length
          ? p.sets.map((s, i) => `<small class="muted">S${i+1}: ${s.pA}-${s.pB}</small>`).join(' ')
          : '';
        return `<div class="fixture-item jugado">
          ...
          <div class="fixture-teams">
            ...
            <span class="fixture-vs">${usarSets ? `${p.sets_a}:${p.sets_b}` : p.ganador==='A'?'G':'P'} — ${p.ganador==='B'?'G':'P'}</span>
            <span class="${p.ganador==='B'?'team-win':''}">${esc(p.equipo_b)}</span>
          </div>
          ${sets ? `<div style="display:flex;gap:.4rem;flex-wrap:wrap;font-size:.8rem">${sets}</div>` : ''}
...
function renderFixturePublico(el, equipos, partidos, cfg) {
  ...
  if (!noms.length) {
    el.innerHTML = '<p class="empty">No hay equipos registrados.</p>';
    return;
  }
